...
def userviewdietplan(request):
    ...
    if uid and request.session.get("usertype") == "User":
        try:
            ...
            # Fetch specifically personalized plans for this user (is_custom=True)
            context['personalized_plans'] = Dietplan.objects.filter(custom_for_user=user_login, is_custom=True)
            
            # Fetch general plans matching the user's goal (is_custom=False)
            if user_goal:
                context['recommended_plans'] = Dietplan.objects.filter(category=user_goal, is_custom=False)
            else:
                context['recommended_plans'] = Dietplan.objects.filter(is_custom=False)
...
def nutriviewdietplan(request):
    ...
    if not uid or request.session.get("usertype") != "Nutritionist":
        return redirect('/login')
    print(uid)
    userData = Dietplan.objects.filter(loginid=uid)
    print(userData)
...
def suggest_plan(request):
    ...
    return render(request, 'nutritionists/suggest_plan.html', {'request_obj': req_obj, 'diet_plans': diet_plans})
